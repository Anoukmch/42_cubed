...
void init_player(t_vars *vars)
{
	...
	ft_memset(vars->player_img->pixels, 255, vars->player_img->width * vars->player_img->height * sizeof(int));
	mlx_image_to_window(vars->mlx, vars->player_img, vars->p_width_pos, vars->p_height_pos); //position in pixel/in window
}
...
void view_starting_direction(t_vars *vars)
{
	...
	vars->dir_x = 0;
	vars->dir_y = -1;
	vars->view_img = mlx_new_image(vars->mlx, vars->m_width * 2, vars->m_height * 2);
